...
import { fetchBothMilestoneQuiz, submitBothMilestoneQuiz } from '@/lib/api/both'
import type { BothQuiz } from '@/lib/api/both'
...
export function BothMilestoneQuizCard({ milestoneId, milestoneTitle, pathId, onPass }: Props) {
  const [quiz, setQuiz] = useState<BothQuiz | null>(null)
  ...
  useEffect(() => {
    fetchBothMilestoneQuiz(milestoneId, pathId)
      .then(setQuiz)
  ...
  const submit = async () => {
    ...
    setLoading(true)
    try {
      const res = await submitBothMilestoneQuiz(milestoneId, pathId, answers)
      setResult(res)
